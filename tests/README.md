...
`product-matrix.test.mjs` — הפירוט לפי מוצר × ימים במרכזת (v64): מבנה
העמודות ואות היום, מיון הקודים כמחרוזת כמו בחשבונית, ההחזרה שיושבת בתא של
אותו יום, מה לא נכנס לטבלה (פיקדון, שורה כפולה למוצר בלי מזהה), סגירת
הסכומים בשורות ובעמודות, סימוני המבצע ושינוי המחיר, ותקופה ריקה.
