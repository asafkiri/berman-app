...
`single-unknown-price.test.mjs` — הנעלם היחיד (v81): כשכל שורות התעודה
ידועות חוץ ממוצר אחד שאין לו אחוז הנחה, חשבון הנייר גוזר את מחיר היחידה
שלו. הבדיקות רצות על המתאם האמיתי ועל השער, ומכסות את הפתרון עצמו, את
רצועת הדיוק שנגזרת מהכמות, את סולם הפשטות של האחוז המוצע, את העדות
התומכת מהקטלוג, את העובדה שהפתרון **אינו** פותח את שער האמון ואינו כותב
למאגר, את קביעת האחוז שמתמחרת מחדש ופותחת אותו באמת, ואת כל המצבים שבהם
אין פתרון יחיד ולכן מוחזר `null`. סעיף שלם מוקדש לשורת מבצע שמודפסת
במחירון — היא יכולה להיות מחויבת בשתי דרכים, ההפרש נופל על הנעלם, ולכן
שתי האפשרויות מוצגות במקום שאחת תיבחר בשקט. `--backup` מריץ את אותם
תרחישים מול קטלוג אמיתי.
