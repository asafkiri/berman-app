...
## v82: הנחה חסרה אינה כשל בצילום

`node --test tests/missing-discount.test.mjs tests/manual-quantities.test.mjs`
בודק שמסך הכמויות הידני מציג ישירות את חישוב ההנחה כשהקריאה מלאה, כולל
בחירה בין חיובי מבצע ושחזור טיוטה. אישור ההנחה פותח את בדיקת הכמויות בלי
צילום חוזר ובלי שינוי בכמויות שנספרו. פענוח חסר, סיכום סותר או תעודה נוספת
שאינה נסגרת עדיין מציגים את אפשרות תיקון הצילום.

## v81: השלמת הנחה חסרה מתוך התעודה

`node --test tests/missing-discount.test.mjs` בודק את מודול האפליקציה המלא:
נעלם יחיד לפי הכמות בנייר, אפס שהוגדר לעומת אחוז חסר, שורות חוזרות, הפרדה
בין תעודות, מבצע מודפס לעומת חיוב רגיל/מלא, בחירת בסיס ושחזורו, אישור ועריכת
ההנחה דרך שני מסכי האפליקציה, שמירה שנכשלה, פענוח שלא נשמר, הצעה שהתיישנה
ולחיצה כפולה. אישור ההנחה אינו הופך את החישוב להוכחה עצמאית למחיר הספק.
